 ...
 *                                  exceptions.
 * Apr 09, 2013  1802     bphillip  Changed to use new query method signatures
 *                                  in SessionManagedDao
 ...
 * @author djohnson
 */
public class SubscriptionRetrievalDao
        extends BaseBandwidthAllocationDao<SubscriptionRetrieval>
        implements ISubscriptionRetrievalDao {
    ...
    private static final String GET_SUBSCRIPTIONRETRIEVAL_BY_PROVIDER_DATASET_STATUS_AND_DATES = GET_SUBSCRIPTIONRETRIEVAL_BY_PROVIDER_AND_DATASET
            + " and sr.status = :status" + " and sr.startTime >= :startDate"
            + " and sr.startTime <= :endDate" + " order by sr.startTime ";

    private static final String DELETE_SUBSCRIPTIONRETRIEVAL_BEFORE_DATE = "delete from SubscriptionRetrieval sr where sr.endTime <= :endDate";
